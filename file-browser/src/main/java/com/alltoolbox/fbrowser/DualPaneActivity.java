...
public class DualPaneActivity extends AppCompatActivity {
    ...
    private final class Pane {
        ...
        void onOpen(FileInfo fi) {
            // 打开前快照当前多选（.sh 运行时可把选中文件路径作为脚本参数）
            List<File> selSnapshot = selectedFiles();
            ...
            if (fi.isDirectory()) {
                ...
                setActiveSide(side);
                navigate(target);
            } else if (com.alltoolbox.core.file.FileUtil
                    .getExtension(fi.getName()).equals("sh")) {
                // .sh：工作目录 = 本栏真实目录；多选文件作为参数
                DualPaneActivity.this.openShell(fi.getFile(), selSnapshot,
                        new File(Pane.this.currentPath));
            } else {
    ...
    /** .sh 脚本打开入口：弹出 编辑/运行/设权限，选中文件路径作为脚本入参。 */
    private void openShell(File f, List<File> sel, File workDir) {
        List<String> args = new ArrayList<>();
        for (File x : sel) {
            if (!x.equals(f)) args.add(x.getAbsolutePath());
        }
        com.alltoolbox.fbrowser.shell.ShellTools.showDialog(this, f, workDir, args);
    }
